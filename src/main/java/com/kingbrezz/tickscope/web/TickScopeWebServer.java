...
public final class TickScopeWebServer {
    ...
    private void route(HttpExchange ex) throws IOException {
        ...
        switch (path) {
            case "/api/health" -> send(ex, 200, Map.of("success", true, "status", "ok", "plugin", "TickScope", "version", plugin.getDescription().getVersion(), "server", plugin.getServer().getName(), "endpoint", "/api/v1/all"));
            case "/api/status" -> send(ex, 200, ApiData.status(plugin));
            case "/api/all" -> send(ex, 200, ApiData.all(plugin));
